...
def getNDLItemsByKeywords(keyword):

    target_url = 'https://iss.ndl.go.jp/api/opensearch?title='+keyword
    ...
    pd.DataFrame(items).to_excel(keyword+'.xlsx', sheet_name=keyword, columns=columns, encoding="cp932")
    print('Mission Accomplished. Please Download the Output File.')
```

3. 上のメニューバーにある「＋」ボタンを押すと、スクリプトをペーストしたボックスの下に、新しいボックスが追加されます。新しいボックスに、下のコードを貼り付けたのち、「キーワード」の部分に検索語（複数入れる場合はスペースで区切る）を入れてください。

```python
getNDLItemsByKeywords('キーワード')
```

4. 上二つのボックス（スクリプトをペーストしたボックス）を実行（ボックス横の再生ボタンを押す）すると、処理がスタートします。処理が終わると、画面左のファイルリストに「キーワード.xlsx」というExcelファイルが現れます。右クリックで「ダウンロード」を選択してください。
